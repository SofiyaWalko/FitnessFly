...
function Program() {
	...
	function startProgram() {
		const user_id = localStorage.getItem("user_id");
		if (!user_id) {
			alert("Для начала программы необходимо зарегистрироваться");
			return;
		}
	...
	function handleComplete() {
		...
		fetch("http://fitnessfly.local/api/training/completeTraining.php", {
			...
			.then((data) => {
				...
				alert(
					`Тренировка завершена! Вам начислено ${data.points_added} Fitcoins`,
				);
						...
						return (
							<button
								...
								onClick={() => {
									//программа не начата
									if (!isStarted) {
										alert("Сначала начните программу");
										return;
									}

									if (isLocked) {
										alert("Сначала завершите текущий день");
										return;
									}
